...
class PushNotification {
  ...
  loadPushes(success, error = () => {}) {
      if (typeof success != 'function') {
         console.log("loadPushes error: successCallback must be a function");
         return;
      }
  ...
  status(success = (status) => {console.log('Push notifications are ' + status);}) {
      if (typeof(success) != "function") {
          success = (status) => {console.log('Push notifications are ' + status);}
      }
      exec(success, (error) => {console.log(error);}, 'PushNotification', 'checkState', []);
  }
}
